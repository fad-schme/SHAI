...
the command *is* the approval. See `connectors.md` for the manifest schema
and the per-call approval gate this feeds.
...
| `--boundary` / `-b NAME` | — | Filter: `input_scan`, `tool_call_gate`, `tool_dispatch_check`, `tool_result_scan`, `output_scan`, `file_scan`, `mcp_metadata_scan`, `mcp_source_onboarding`, `system`. |
