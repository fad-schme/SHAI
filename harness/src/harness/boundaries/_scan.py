...
async def run_scan(
    ...
    *,
    ...
    for i, (scanner, result) in enumerate(zip(scanners, raw_results)):
        ...
        effective_action = scanner_actions[i] if scanner_actions[i] is not None else boundary_action
        redact_with      = scanner_redact_withs[i]
        ...
        if action == ScanAction.BLOCK:
            ...
            if result is not None:
                current_text = _apply_redaction(
                    current_text, triggering, result, redact_with
                )
            else:
                # No scanner result — fall back to block
